...
//Title
echo '<h3 style="margin-bottom: 2rem;">Job Experience Statistics</h3>';
...
if ($pet['farmEXP'] > 1000) {
    ...
    $num = ceil($pet['farmEXP']);
}
echo '<label for="Farmer" class="jobtitle">' . $farmname . '</label><br>';
...
if ($pet['exploreEXP'] > 1000) {
    $num = 1000;
} else {
    $num = ceil($pet['exploreEXP']);
}
echo '<label for="Explorer" class="jobtitle" >' . $explorename . '</label><br>';
echo '<p>Higher Level Finds More Items When Exploring</p>';
//Calculate Values for Each Job. Display Radio Button then Job
...
if ($pet['craftEXP'] > 1000) {
    ...
    $num = ceil($pet['craftEXP']);
}
echo '<label for="Crafter" class="jobtitle">' . $craftname . '</label><br>';
